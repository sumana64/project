<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	pageEncoding="ISO-8859-1"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="ISO-8859-1">
<title>Insert title here</title>
<link
	href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css"
	rel="stylesheet"
	integrity="sha384-9ndCyUaIbzAi2FUVXJi0CjmCapSmO7SnpJef0486qhLnuZ2cdeRhO02iuK6FUUVM"
	crossorigin="anonymous">
<style type="text/css">
html, body {
	height: 100%;
	box-sizing: border-box;
}

footer {
	overflow: auto;
	padding-bottom: 100px;
	position: relative;
	height: 100%;
	margin-top: 30px;
	clear: both;
	box-sizing: inherit;
}
</style>
<script>
function validateFname() {

	console.log("Name function is running")
	const button = document.getElementById('button')
	const fname = document.getElementById('inputFname').value
	console.log(fname)
	console.log(fname.length)

	if (fname.length>3 && fname.length <= 30) {

		document.getElementById("nameErr").innerHTML = "<span style='color:green'>firstName is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("nameErr").innerHTML = "<span style='color:red'>firstName is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}
function validateMname() {

	console.log("Name function is running")
	const button = document.getElementById('button')
	const mname = document.getElementById('inputMname').value
	console.log(mname)
	console.log(mname.length)

	if (mname.length >=2 && mname.length <= 30) {

		document.getElementById("mnameErr").innerHTML = "<span style='color:green'>MiddleName is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("mnameErr").innerHTML = "<span style='color:red'>MiddleName is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}

function validateLname() {

	console.log("Name function is running")
	const button = document.getElementById('button')
	const lname = document.getElementById('inputLname').value
	console.log(lname)
	console.log(lname.length)

	if (lname.length >=1 && lname.length <= 30) {

		document.getElementById("lnameErr").innerHTML = "<span style='color:green'>LastName is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("lnameErr").innerHTML = "<span style='color:red'>LastName is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}
function validateCountry() {

	console.log("Name function is running")
	const button = document.getElementById('button')
	const country = document.getElementById('inputCountry').value

	if (country.length > 3 && country.length <= 100) {

		document.getElementById("countryErr").innerHTML = "<span style='color:green'>country is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("countryErr").innerHTML = "<span style='color:red'>country is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}
function  validateAge(){

	console.log("Name function is running")
	const button = document.getElementById('button')
	const age = document.getElementById('inputAge').value

	if (age!=0 && age>=18) {

		document.getElementById("ageErr").innerHTML = "<span style='color:green'>Age is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("ageErr").innerHTML = "<span style='color:red'>Age is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}
function  validateQualification(){

	console.log("Name function is running")
	const button = document.getElementById('button')
	const qualification = document.getElementById('inputQualification').value

	if (qualification.length>=2 && qualification.length <= 100) {

		document.getElementById("qualificationErr").innerHTML = "<span style='color:green'>qualification is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("qualificationErr").innerHTML = "<span style='color:red'>qualification is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}
function validatePlaceOfBirth(){

	console.log("Name function is running")
	const button = document.getElementById('button')
	const placeOfBirth = document.getElementById('inputPlaceOfBirth').value

	if (placeOfBirth.length>3 && placeOfBirth.length<120) {

		document.getElementById("placeOfBirthErr").innerHTML = "<span style='color:green'>placeOfBirth is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("placeOfBirthErr").innerHTML = "<span style='color:red'>placeOfBirth is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}

	function validateRtoLocation(){

	console.log("Name function is running")
	const button = document.getElementById('button')
	const rtoLocation= document.getElementById('inputRtoLocation').value

	if (rtoLocation.length>3 && rtoLocation.length<120) {

		document.getElementById("rtoLocationErr").innerHTML = "<span style='color:green'>rtoLocation is valid</span>"
		button.removeAttribute("disabled")

	} else {

		document.getElementById("rtoLocationErr").innerHTML = "<span style='color:red'>rtoLocation is Invalid</span>"
		button.setAttribute("disabled", "")
	}

}	
	
	function validatePinCode() {

			console.log("Name function is running")
			const button = document.getElementById('button')
			const pinCode= document.getElementById('inputPinCode').value

			if (pinCode!=0) {

				document.getElementById("pinCodeErr").innerHTML = "<span style='color:green'>pinCode is valid</span>"
				button.removeAttribute("disabled")

			} else {

				document.getElementById("pinCodeErr").innerHTML = "<span style='color:red'>pinCode is Invalid</span>"
				button.setAttribute("disabled", "")
			}

		}
	
	function validateEmail(){
		    const em = document.getElementById('inputEmail').value;
		    const button = document.getElementById('button');
		    if (em.match(/^[a-z]+@[a-z]+\.[a-z]{3}$/)) {
		      document.getElementById("emailErr").innerHTML = "<span style='color: green'>Email is valid</span>";
		      button.removeAttribute("disabled");
		   
		    } else {
		      document.getElementById("emailErr").innerHTML = "<span style='color: red'>Invalid Email format</span>";
		      button.setAttribute("disabled", "");
		    }
		  	
	}
	function validatePhoneNo(){
			
		    console.log("Number function is running")
		    const button = document.getElementById('button')
		    const phoneNo = document.getElementById('inputPhoneNo').value
		   
		    if(phoneNo.length === 10){
		    	
		    	document.getElementById("phoneNoErr").innerHTML="<span style='color:green'>phoneNo is valid</span>"
		    	button.removeAttribute("disabled")
		   
		    }else{
		    	
		    	document.getElementById("phoneNoErr").innerHTML="<span style='color:red'>phoneNo is Invalid</span>"
		        button.setAttribute("disabled","")

		    }
		  
		}
	function validateAadharNo() {

		   console.log("Number function is running")
		    const button = document.getElementById('button')
		    const aadharNo = document.getElementById('inputAadharNo').value
		    
		    if(aadharNo!=0 && aadharNo>=12){
		    	
		    	document.getElementById("aadharNoErr").innerHTML="<span style='color:green'>aadharNo is valid</span>"
		    	button.removeAttribute("disabled")
		   
		    }else{
		    	
		    	document.getElementById("aadharNoErr").innerHTML="<span style='color:red'>aadharNo is Invalid</span>"
		        button.setAttribute("disabled","")

		    }

		}
	function validatePermanentAddress() {

			console.log("Name function is running")
			const button = document.getElementById('button')
			const permanentAddress = document.getElementById('inputPermanentAddress').value

			if (permanentAddress.length > 3 && permanentAddress.length <= 100) {

				document.getElementById("permanentAddressErr").innerHTML = "<span style='color:green'>PermanentAddress is valid</span>"
				button.removeAttribute("disabled")

			} else {

				document.getElementById("permanentAddressErr").innerHTML = "<span style='color:red'>PermanentAddress is Invalid</span>"
				button.setAttribute("disabled", "")
			}

		}	
	
		function validateCurrentAddress() {

			console.log("Name function is running")
			const button = document.getElementById('button')
			const currentAddress = document.getElementById('inputCurrentAddress').value

			if (currentAddress.length > 3 && currentAddress.length <= 100) {

				document.getElementById("currentAddressErr").innerHTML = "<span style='color:green'>currentAddress is valid</span>"
				button.removeAttribute("disabled")

			} else {

				document.getElementById("currentAddressErr").innerHTML = "<span style='color:red'>currentAddress is Invalid</span>"
				button.setAttribute("disabled", "")
			}

		}

</script>
</head>
<body>
	<nav class="navbar navbar-expand-lg bg-warning">
		<div class="container-fluid">

			<a class="navbar-brand" href="#"> <img
				src="data:image/jpeg;base64,"
				alt="logo" width="50" height="50">
			</a> <img
				src="https://e7.pngegg.com/pngimages/679/69/png-clipart-home-assistant-computer-icons-home-automation-kits-amazon-echo-home-blue-logo.png"
				alt="logo" width="50" height="50"> </a>

			<div class="collapse navbar-collapse" id="navbarSupportedContent">
				<ul class="navbar-nav me-auto mb-2 mb-lg-0">

					<li class="nav-item"><a class="nav-link active"
						aria-current="page" href="index.jsp">Home</a></li>

				</ul>

				<ul class="nav nav-pills nav-fill nav justify-content-end">
					<li class="nav-item"><a class="nav-link active"
						aria-current="page" href="SignIn.jsp">SignOut</a></li>
				</ul>

			</div>
	</nav>
	
<!-- 	<div class="col-md-3">
				<label for="inputApplication" class="form-label">ApplicationNo</label> <input
					type="text" name="applicationNo" class="form-control"
					required="required">
					
			</div> -->
			
			<h4>${message}</h4>
			<h5>${message1}</h5>

	<section
		class="container my-2  w-200 bg-success-subtle border-dark border-5 mt-5 text-black py-3">
		<h3 span style="text-align: center">REGISTRATION FORM</h3>
		<form action="register" method="post" class="row g-3"
			required="required">

			<div class="col-md-4">
				<label for="inputFname" class="form-label">Fname</label> <input
					type="text" name="fname" class="form-control" onblur="validateFname()" id="inputFname"
					required="required">
					<span id="nameErr"></span>
			</div>

			<div class="col-md-4">
				<label for="inputMname" class="form-label">Middle Name</label> <input
					type="text" name="mname" class="form-control" onblur="validateMname()" id="inputMname"
					required="required">
					<span id="mnameErr"></span>
			</div>

			<div class="col-md-4">
				<label for="inputLname" class="form-label">Lname</label> <input
					type="text" name="lname" class="form-control" onblur="validateLname()" id="inputLname"
					required="required">
					<span id="lnameErr"></span>
			</div>

			<div class="col-md-4">
				<legend class="col-form-label col-sm-2 pt-0">Gender</legend>
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="gender"
						id="inlineRadio1" value="male"> <label
						class="form-check-label" for="inlineRadio1" required="required">Male</label>
				</div>


				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="gender"
						id="inlineRadio2" value="female"> <label
						class="form-check-label" for="inlineRadio2" required="required">Fmale</label>
				</div>

				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="gender"
						id="inlineRadio3" value="others"> <label
						class="form-check-label" for="inlineRadio3" required="required">Others</label>
				</div>
			</div>
			<div class="col-md-4">
				<label for="inputState" class="form-label" required="required">State</label>
				<select id="inputState" onblur = "validateState()" name="state" class="form-select">
					<option value="">--- Select State Name ---</option>
					<option value="Andaman and Nicobar">Andaman and Nicobar</option>
					<option value="Arunachal Pradesh">Arunachal Pradesh</option>
					<option value="Andhra Pradesh">Andhra Pradesh</option>
					<option value="Assam">Assam</option>
					<option value="Bihar">Bihar</option>
					<option value="Chandigarh">Chandigarh</option>
					<option value="Chhattisgarh">Chhattisgarh</option>
					<option value="Delhi">Delhi</option>
					<option value="Goa">Goa</option>
					<option value="Gujarat">Gujarat</option>
					<option value="Haryana">Haryana</option>
					<option value="Himachal Pradesh">Himachal Pradesh</option>
					<option value="Jammu and Kashmir">Jammu and Kashmir</option>
					<option value="Jharkhand">Jharkhand</option>
					<option value="karnataka">Karnataka</option>
					<option value="Kerala">Kerala</option>
					<option value="Ladakh">Ladakh</option>
					<option value="Maharashtra">Maharashtra</option>
					<option value="Manipur">Manipur</option>
					<option value="Madhya Pradesh">Madhya Pradesh</option>
					<option value="Meghalaya">Meghalaya</option>
					<option value="Mizoram">Mizoram</option>
					<option value="Nagaland">Nagaland</option>
					<option value="Odisha">Odisha</option>
					<option value="Puducherry">Puducherry</option>
					<option value="Punjab">Punjab</option>
					<option value="Rajasthan">Rajasthan</option>
					<option value="Sikkim">Sikkim</option>
					<option value="Tamil Nadu">Tamil Nadu</option>
					<option value="Tripura">Tripura</option>
					<option value="UT of DNH and DD">UT of DNH and DD</option>
					<option value="Uttarakhand">Uttarakhand</option>
					<option value="Uttar Pradesh">Uttar Pradesh</option>
					<option value="West Bengal">West Bengal</option>

				</select>
				<span id="stateErr"></span>
			</div>
			<div class="col-md-4">
				<label for="inputCountry" class="form-label">Country</label> <input
					type="text" class="form-control" name="country" onblur="validateCountry()" id="inputCountry"
					required="required">
					<span id="countryErr"></span>
			</div>

			<div class="col-md-4">
				<label for="inputDateOfBirth" class="form-label">Date Of
					Birth</label> <input type="date" class="form-control" name="dob"
					id="inputDateOfBirth" required="required">
					
			</div>

			<div class="col-md-4">
				<label for="inputAge" class="form-label">Age</label> <input
					type="number" class="form-control" onblur="validateAge()" name="age" id="inputAge"
					required="required">
					<span id="ageErr"></span>
			</div>

			<div class="col-md-4">
				<label for="inputBloodGroup" class="form-label">Blood Group</label>
				<select id="inputBloodGroup" name="bloodGroup" class="form-select"
					required="required">
					<option selected>Select blood group</option>
					<option value="A+">A+</option>
					<option value="A-">A-</option>
					<option value="AB+">AB+</option>
					<option value="AB-">AB-</option>
					<option value="B+">B+</option>
					<option value="B-">B-</option>
					<option value="O+">O+</option>
					<option value="O-">O-</option>

				</select>
			</div>

			<div class="col-md-3">
				<label for="inputQualification" class="form-label">Qualification</label>
				<input type="text" name="qualification" class="form-control" onblur="validateQualification()"
					id="inputQualification" required="required">
					<span id="qualificationErr"></span>
			</div>

			<div class="col-md-3">
				<label for="inputPlaceOfBirth" class="form-label">PlaceOfBirth</label>
				<input type="text" name="placeOfBirth" class="form-control" onblur="validatePlaceOfBirth()"
					id="inputPlaceOfBirth" required="required">
					<span id="placeOfBirthErr"></span>
			</div>

			<div class="col-md-3">
				<label for="inputRtoLocation" class="form-label">RtoLocation</label>
				<input type="text" class="form-control" name="rtoLocation" onblur="validateRtoLocation()"
					id="inputRtoLocation" required="required">
					<span id="rtoLocationErr"></span>
			</div>

			<div class="col-md-3">
				<label for="inputPinCode" class="form-label">PinCode</label> <input
					type="text" class="form-control" onblur="validatePinCode()" name="pinCode" id="inputPinCode"
					required="required">
					<span id="pinCodeErr"></span>
			</div>

			<div class="col-md-4">
				<label for="inputEmail" class="form-label">Email</label> <input
					type="email" class="form-control" onblur="validateEmail()" name="email" id="inputEmail"
					required="required">
					<span id="emailErr"></span>
			</div>
			<div class="col-md-4">
				<label for="inputPhoneNo" class="form-label">PhoneNo</label> <input
					type="tel" class="form-control" onblur="validatePhoneNo()" name="phoneNo" id="inputPhoneNo"
					required="required">
					<span id="phoneNoErr"></span>
			</div>
			<div class="col-md-4">
				<label for="inputAadharNo" class="form-label">AadharNo</label> <input
					type="tel" class="form-control" onblur="validateAadharNo()" name="aadharNo" id="inputAadharNo"
					required="required">
					<span id="aadharNoErr"></span>
			</div>
			<div class="col-6">
				<label for="inputPermanentAddress" class="form-label">Permanent
					Address</label> <input type="text" name="permanentAddress"
					class="form-control" onblur="validatePermanentAddress()" id="inputPermanentAddress" required="required">
					<span id="permanentAddressErr"></span>
			</div>

			<div class="col-6">
				<label for="inputCurrentAddress" class="form-label">Current
					Address</label> <input type="text" class="form-control"
					id="inputCurrentAddress" onblur="validateCurrentAddress()" name="currentAddress" required="required">
				  <span id="currentAddressErr"></span>
					
			</div>

			<div class="col-6">
				<hr>
			</div>
			<div class="col-1">Declaration</div>
			<div class="col-5">
				<hr>
			</div>

			<span style="color: red">Fill-up above form details before
				submitting the application</span>

			<div class="col-12">
				<ol>
					<li>
						<div class="col-md-12">
							I am willing to donate my organs, in case of accidental death?
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									name="inlineRadioOptions" id="inlineRadio1" value="option1">
								<label class="form-check-label" for="inlineRadio1">Yes</label>
							</div>
							<div class="form-check form-check-inline">
								<input class="form-check-input" type="radio"
									name="inlineRadioOptions" id="inlineRadio2" value="option2">
								<label class="form-check-label" for="inlineRadio2">No</label>
							</div>
						</div>
					</li>
					<li><div class="col-md-12">
							I here by declare that to the best of my knowledge and belief the
							particulars given above are true
							<div class="form-check">
								<input class="form-check-input" type="checkbox" value=""
									id="flexCheckDefault"><label class="form-check-label"
									for="flexCheckDefault"></label>
							</div>
						</div></li>

				</ol>
			</div>
			<div class="col-md-4">
				<label for="inputPhoneNo" name="phoneNo" class="form-label">AlternatePhoneNo</label>
				<input type="tel" class="form-control" id="inputPhoneNo">
			</div>
			<div class="col-md-4"></div>
			<div class="col-md-4"></div>
			<br>

			<div class="col-1">
				<button type="submit" id="button" class="btn btn-success">Submit</button>
			</div>
			<div class="col-1">
				<button type="submit" value="refresh" class="btn btn-success">Refresh</button>
			</div>
			<div class="col-1">
				<button type="submit" value="cancel" class="btn btn-success">Cancel</button>
			</div>
		</form>

	</section>
</body>
<div id="footer">
	<%@include file="footer.jsp"%>
</div>

</html>